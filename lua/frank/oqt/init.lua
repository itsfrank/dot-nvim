    ...
    select = function(list_item, _, _)
        local make_task = function()
            return overseer.new_task({
                name = list_item.value.name,
                cmd = list_item.value.cmd,
                components = { -- required because "default" causes errors
...
function oqt.float_last_task()
    ...
    if #tasks == 0 then
        vim.notify("no recent tasks", vim.log.levels.INFO)
        return
    end
...
function oqt.setup()
    ...
    vim.keymap.set("n", "<leader>to", function()
        oqt.float_last_task()
    end, { desc = "last [T]ask [O]utput - harpoon overseer quick tasks" })
